...
async function loadConversionTemplate(): Promise<string> {
  const templatePath = join(TEMPLATE_DIR, "rule-conversion-markdown.txt");
...
async function convertFile(filePath: string, outputDir: string): Promise<void> {
  ...
  const promptContent = template
    ...
    .replace("{tags}", ruleData.tags.join(", "))
  ...
  const markdownContent = data.content[0].text.replace(/^```markdown\n/, '').replace(/\n```$/, '');
...
async function main() {
  ...
  console.log(`Starting TypeScript to Markdown conversion...`);
  console.log(`Input directory: ${inputDir}`);
  console.log(`Output directory: ${outputDir}`);
  await log(`Starting TypeScript to Markdown conversion...`, "importer.log");
