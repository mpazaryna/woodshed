/**
 * @file aic_prompts.ts
 * @description Generates context-aware artifacts using domain-specific prompts with full primitive inclusion
 */

import { parse } from "https://deno.land/std@0.208.0/flags/mod.ts";
import { join } from "https://deno.land/std@0.208.0/path/mod.ts";
import { XMLGenerator } from './xml_generator.ts';
import { readFile, writeFile, ensureDirectory, getEnvVariable } from "./utils.ts";

// Constants
const DATA_DIR = "data";
const OUTPUT_PATH = join(DATA_DIR, "tmp", "aic.xml");
...
async function main() {
  try {
    ...
    const xmlOutput = XMLGenerator.generateStackXML(matchedPrimitives, aiResult);
    await ensureDirectory(join(DATA_DIR, "tmp"));
    await writeFile(OUTPUT_PATH, xmlOutput);
...
async function writeEvaluation(evalContent: string) {
  await Deno.mkdir('data/evals', { recursive: true });
  await Deno.writeTextFile('data/evals/aic.txt', evalContent);
  console.log('Evaluation written to data/evals/aic.txt');
}

async function writeXMLOutput(xmlOutput: string) {
  await Deno.mkdir(join(DATA_DIR, "tmp"), { recursive: true });
  await Deno.writeTextFile(OUTPUT_PATH, xmlOutput);
